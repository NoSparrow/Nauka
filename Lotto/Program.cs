...
class Program
{
    ...
    static bool Function5_ProcessData()
    {
        ...
        string processedFile = Path.Combine(folderPath, "Przetworzone.txt"); // Plik do śledzenia przetworzonych losowań.
        string resultsFile = Path.Combine(folderPath, "Wyniki.txt");         // Plik do zapisywania wyników analizy Z-score.

        // Odczytaj dane z głównego pliku PobraneDane.txt (pomijamy pierwszą linię, która jest nagłówkiem).
        if (!File.Exists(filePath))
        {
        ...
        // Odczytaj już przetworzone losowania z pliku Przetworzone.txt.
        ...
        {
            // Jeśli plik Przetworzone.txt nie istnieje, utwórz go.
            File.Create(processedFile).Close();
        }

        // Znajdź losowania, które nie zostały jeszcze przetworzone (czyli te z pliku PobraneDane.txt, których nie ma w processedSet).
        var unprocessed = allData.Where(line => !processedSet.Contains(line)).ToList();

        if (unprocessed.Count == 0)
        {
            // Jeśli nie ma nowych losowań, pytamy, czy kontynuować program.
        ...
        // Wartości statystyczne potrzebne do obliczeń (Z-score).
        double mean = 25.0; // Średnia teoretyczna.
        double stdDev = 14.1421356237; // Odchylenie standardowe teoretyczne.

        List<string> resultLines = new List<string>();
        List<string> processedLines = new List<string>();

        // Przetwarzanie losowań sekwencyjnie (jedno po drugim).
        foreach (var line in unprocessed)
        {
            // Podziel linię na części (data i liczby).
            var parts = line.Split('|');
            string date = parts[0].Trim();

            // Wczytanie wylosowanych liczb.
            var numbers = parts[1].Split(new char[] { ' ' }, StringSplitOptions.RemoveEmptyEntries)
                                    .Select(int.Parse).ToList();

            // Oblicz Z-score dla każdej wylosowanej liczby.
            // Z-score = (wartość - średnia) / odchylenie standardowe.
            var zScores = numbers.Select(n => Math.Round((n - mean) / stdDev, 10)).ToList();

            // Oblicz średni Z-score dla całego losowania i Z-score dla kombinacji.
            double avgZ = Math.Round(zScores.Average(), 10);
            double comboZ = Math.Round(zScores.Sum() / Math.Sqrt(zScores.Count), 10);

            // Stworzenie linii z wynikami Z-score.
            string result = $"{date} | {string.Join(" ", numbers)} | {string.Join(" | ", zScores.Select(z => z.ToString("F10")))} | {avgZ:F10} | {comboZ:F10}";
            resultLines.Add(result);
            processedLines.Add(line); // Dodanie linii do listy przetworzonych.

            Console.WriteLine($"Przetworzono losowanie: {date}");
            
            // Interakcja z użytkownikiem: "naciśnij ENTER, aby kontynuować lub wpisz 'stop'".
        ...
        // Zapisanie wyników analizy do pliku Wyniki.txt i przetworzonych linii do Przetworzone.txt.
        // Użycie AppendAllLines dodaje linie na końcu istniejącego pliku.
